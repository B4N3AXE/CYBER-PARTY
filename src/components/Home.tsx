...
import { Flame, Volume2, VolumeX, HelpCircle, User, Sparkles, Smile, PlusCircle, Zap, ArrowRight, AlertCircle, X, Gamepad2, Loader2 } from 'lucide-react';
...
export default function Home({ onJoin }: { onJoin: (roomId: string, name: string, avatar: string) => void }) {
  ...
  return (
    <div className="flex-1 flex flex-col justify-between relative select-none">
      {/* Top Bar Navigation */}
      <header className="relative z-20 w-full max-w-7xl mx-auto px-6 py-5 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-pink-500 via-purple-600 to-cyan-400 p-[1px] shadow-[0_0_15px_rgba(236,72,153,0.5)]">
          ...
          <div>
            <span className="font-cyber text-lg tracking-wider font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-pink-400 via-purple-300 to-cyan-300">CYBERSPIN</span>
            <span className="ml-2 text-[10px] font-mono uppercase px-2 py-0.5 rounded-full bg-cyan-500/10 text-cyan-300 border border-cyan-500/30 tracking-widest">v2.4 ONLINE</span>
          </div>
        </div>
        
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-slate-900/60 border border-slate-700/60 text-xs font-medium text-slate-300 backdrop-blur-md shadow-inner">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>
            <span className="text-slate-400">Sunucu:</span>
            <span className="text-emerald-300 font-mono">18ms (TR-IST)</span>
          </div>
          <button onClick={() => setIsMuted(!isMuted)} className="p-2.5 rounded-xl bg-slate-800/60 hover:bg-slate-700/60 border border-slate-700/60 text-slate-300 hover:text-cyan-300 transition backdrop-blur-md" title="Ses Aç/Kapat">
            {isMuted ? <VolumeX className="w-4 h-4" /> : <Volume2 className="w-4 h-4" />}
          </button>
          <button onClick={() => setShowRules(true)} className="p-2.5 rounded-xl bg-slate-800/60 hover:bg-slate-700/60 border border-slate-700/60 text-slate-300 hover:text-pink-300 transition backdrop-blur-md" title="Nasıl Oynanır?">
        ...
        <div className="w-full max-w-[460px]">
          <div className="glass-panel rounded-3xl p-7 sm:p-9 relative overflow-hidden transition-all duration-300">
            ...
            <div className="flex flex-col items-center text-center mb-7">
              ...
              <h1 className="font-cyber text-3xl sm:text-4xl font-black tracking-wider uppercase neon-text-gradient mt-1">
                CYBER<span className="text-cyan-400 drop-shadow-[0_0_15px_rgba(34,211,238,0.7)]">SPIN</span>
              </h1>
          ...
          <div className="mt-6 flex flex-wrap items-center justify-center gap-4 text-xs text-slate-400">
            <div className="flex items-center gap-1.5 bg-slate-900/60 px-3 py-1.5 rounded-lg border border-slate-800/80 backdrop-blur-md">
              <span className="w-3.5 h-3.5 text-cyan-400 flex items-center justify-center">✓</span>
              <span>Gizlilik Korumalı</span>
            </div>
            <div className="flex items-center gap-1.5 bg-slate-900/60 px-3 py-1.5 rounded-lg border border-slate-800/80 backdrop-blur-md">
              <span className="w-3.5 h-3.5 text-pink-400 flex items-center justify-center">📱</span>
      ...
      {/* Global Footer Stats */}
      <footer className="relative z-20 w-full max-w-7xl mx-auto px-6 py-4 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-500 border-t border-slate-900/60 shrink-0">
