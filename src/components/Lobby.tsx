      ...
      <main className="flex-1 w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-12 gap-6 items-start overflow-y-auto custom-scrollbar">
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            >
              {selectedGame === 'truth' && (
                <div className="absolute -top-3 right-6 bg-pink-500 text-white font-display font-black text-xs px-3 py-1 rounded-full shadow-md flex items-center gap-1">
                  <Check className="w-3.5 h-3.5 stroke-[3]" /> SEÇİLİ OYUN
                </div>
              )}
              <div className="flex items-start justify-between">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-pink-500 to-purple-600 flex items-center justify-center text-3xl shadow-lg shadow-pink-500/30">
                  🍾
                </div>
              ...
              <div>
                <h3 className="font-display font-extrabold text-xl text-white mb-1">CyberTruth & Dare</h3>
                <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                  Şişe ve çark çevirmece! Ekran dönsün, cesur sorular ve komik görevlerle parti kahkahaya boğulsun.
                </p>
              </div>
              <div className="pt-3 border-t border-white/10 flex items-center justify-between">
                <div className="flex items-center gap-1.5 text-xs text-pink-400 font-semibold">
                  <Flame className="w-4 h-4" />
                  <span>Kahkaha & Görevler</span>
                </div>
                {selectedGame === 'truth' ? (
                  ...
                  <button className="px-4 py-2 rounded-xl bg-pink-500/20 hover:bg-pink-500 border border-pink-400/40 hover:border-transparent text-pink-200 hover:text-white text-xs font-bold transition flex items-center gap-1">
            ...
            >
              {selectedGame === 'lexis' && (
                <div className="absolute -top-3 right-6 bg-cyan-400 text-slate-950 font-display font-black text-xs px-3 py-1 rounded-full shadow-md flex items-center gap-1">
                  <Check className="w-3.5 h-3.5 stroke-[3]" /> SEÇİLİ OYUN
                </div>
              )}
              <div className="flex items-start justify-between">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-cyan-400 to-blue-600 flex items-center justify-center text-3xl shadow-lg shadow-cyan-500/30">
                  🔤
                </div>
                <span className="text-xs font-bold px-3 py-1 rounded-full bg-cyan-500/20 border border-cyan-400/40 text-cyan-300">
                  2-8 Oyuncu
                </span>
              </div>
              <div>
                <h3 className="font-display font-extrabold text-xl text-white mb-1">CyberLexis</h3>
                <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                  Kelime & tahmin partisi! Ortaya çıkan harfleri topla, ipuçlarını çöz ve şifreyi ilk bulan sen ol.
                </p>
              </div>
              <div className="pt-3 border-t border-white/10 flex items-center justify-between">
                <div className="flex items-center gap-1.5 text-xs text-cyan-300 font-semibold">
                  <Zap className="w-4 h-4 text-yellow-400" />
                  <span>Hızlı Kelime Savaşı</span>
                </div>
                {selectedGame === 'lexis' ? (
                  ...
                  <button className="px-4 py-2 rounded-xl bg-cyan-500/20 hover:bg-cyan-500 border border-cyan-400/40 hover:border-transparent text-cyan-200 hover:text-white text-xs font-bold transition flex items-center gap-1">
                    <span>Seç</span> <ArrowRight className="w-3.5 h-3.5" />
                  </button>
                )}
              </div>
          ...
          <div className={`bg-[rgba(22,28,45,0.75)] backdrop-blur-md rounded-3xl p-6 sm:p-7 flex flex-col gap-5 border ${selectedGame === 'lexis' ? 'border-cyan-500/20' : 'border-pink-500/20'}`}>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-white/10">
              <div>
                <span className={`text-xs font-bold uppercase tracking-wider ${selectedGame === 'lexis' ? 'text-cyan-400' : 'text-pink-400'}`}>Lobi Durumu</span>
                <h4 className="font-display font-bold text-lg text-white">
                  {selectedGame === 'lexis' ? 'CyberLexis Turu Başlamak Üzere' : 'CyberTruth Turu Başlamak Üzere'}
                </h4>
              </div>
            ...
            {selectedGame === 'lexis' ? (
              <div className="bg-black/30 rounded-2xl p-5 border border-white/5 flex flex-col items-center text-center gap-3">
                ...
                <div className="text-sm sm:text-base font-medium text-slate-200 bg-white/5 px-4 py-2.5 rounded-xl border border-white/10">
                ...
                <div className="flex items-center justify-center gap-1.5 sm:gap-2 my-1">
                  {['K','A','L','E'].map((letter, i) => (
                    <div key={i} className="w-10 h-12 sm:w-12 sm:h-14 rounded-2xl bg-cyan-500/20 border-2 border-cyan-400 text-cyan-300 font-display font-extrabold text-xl sm:text-2xl flex items-center justify-center shadow-lg shadow-cyan-500/20">{letter}</div>
                  ))}
                  <div className="w-10 h-12 sm:w-12 sm:h-14 rounded-2xl bg-white/5 border-2 border-dashed border-cyan-400 text-cyan-400 font-display font-extrabold text-xl sm:text-2xl flex items-center justify-center animate-pulse">?</div>
                  <div className="w-10 h-12 sm:w-12 sm:h-14 rounded-2xl bg-white/5 border border-white/10 text-slate-500 font-display font-extrabold text-xl sm:text-2xl flex items-center justify-center">_</div>
                  <div className="w-10 h-12 sm:w-12 sm:h-14 rounded-2xl bg-white/5 border border-white/10 text-slate-500 font-display font-extrabold text-xl sm:text-2xl flex items-center justify-center">_</div>
                </div>
                <span className="text-xs text-cyan-400/80">İpucu: 8 harfli İngilizce terim (Firewall)</span>
              </div>
            ) : (
              <div className="bg-black/30 rounded-2xl p-5 border border-white/5 flex flex-col items-center text-center gap-3">
                <span className="text-xs font-medium text-slate-400">Tur Özellikleri:</span>
                <div className="flex items-center gap-4 text-slate-300 text-sm bg-white/5 px-4 py-2.5 rounded-xl border border-white/10">
                  <span className="flex items-center gap-1"><Flame className="w-4 h-4 text-pink-400"/> Cesaret Görevleri</span>
                  <span>•</span>
                  <span className="flex items-center gap-1"><Sparkles className="w-4 h-4 text-purple-400"/> Gizli Sorular</span>
                </div>
                <div className="w-20 h-20 rounded-full border-4 border-pink-500/30 flex items-center justify-center mt-2 relative">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 pt-2">
              <button 
                ...
                className={`w-full sm:flex-1 py-4 px-6 rounded-2xl font-display font-extrabold text-lg sm:text-xl shadow-xl flex items-center justify-center gap-3 transition duration-200 ${(isHost ? canStart : myPlayer?.isReady) ? 'bg-gradient-to-r from-emerald-400 via-teal-500 to-cyan-500 text-slate-950 shadow-teal-500/25' : 'bg-white/10 text-white'}`}
              >
                {isHost ? (
                   <><Play className="w-6 h-6 fill-current" /> <span>OYUNU BAŞLAT</span></>
                ) : (
                   <><CheckCircle2 className="w-6 h-6" /> <span>{myPlayer?.isReady ? 'HAZIRIM (BEKLENİYOR)' : 'HAZIR OL'}</span></>
                )}
              </button>
              
              <button onClick={() => { navigator.clipboard.writeText(room.id); alert('Oda Kodu Kopyalandı!'); }} className="w-full sm:w-auto py-4 px-6 rounded-2xl bg-white/10 hover:bg-white/15 border border-white/10 font-display font-bold text-white text-base flex items-center justify-center gap-2 transition">
